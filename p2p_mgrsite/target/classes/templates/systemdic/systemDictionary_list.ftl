...
<head>
	...
	$(function(){
		$(function() {
			...
			//点击添加数据字典addSystemDictionaryBtn弹出模式窗口systemDictionaryModal
					...
					<table class="table">
						...
						<#list pageResult.listData as item>
							<tr>
								...
								<td>
									<!-- data-jsonstr外面是单引号 -->
									<a href="javascript:void(-1);" class="edit_Btn" data-jsonstr='${item.jsonString}'>修改</a>
									<!-- 两种方式 第一种是点击修改后直接将本页面中的jason数据填充到模式弹窗里面 -->
									<!-- 第二种方式 用id通过ajax发送请求到数据库取查询然后再放到模式窗口 -->
								</td>
							</tr>
						</#list>
						</tbody>
